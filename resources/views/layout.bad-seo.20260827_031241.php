...
/* SEO_META_V2 */
$currentSeoPath = current_url_path();
...
foreach ($seoNoindexPrefixes as $prefix) {
    if (
        $currentSeoPath === $prefix
        || str_starts_with($currentSeoPath, $prefix . '/')
    ) {
        $seoNoindex = true;
        break;
    }
...
$seoTitle = trim((string)($title ?? $siteName));
$seoDescription = trim((string)$metaDescription);

$seoOgType = !empty($articleJsonLd)
    ? 'article'
    : 'website';
...
<?php if (!$seoNoindex && $currentSeoPath === '/'): ?>
<?php
$siteRoot = rtrim(
